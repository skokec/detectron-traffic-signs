...
```
cd $DETECTRON/docker
docker build -t detectron/traffic-signs:cuda8-cudnn6 .
```

Run the Detectron image (e.g. for [`BatchPermutationOp test`](tests/test_batch_permutation_op.py)):

```
docker run -it --gpus all detectron/traffic-signs:cuda8-cudnn6 python2 tests/test_batch_permutation_op.py
```
